  ...
  UPDATE public.invoices
     SET delivery_computed_state = new_state,
         ...
         delivery_status = CASE
           WHEN new_state = 'complete' THEN 'delivered'
           WHEN delivery_status = 'delivered' THEN delivery_status
           WHEN new_state IN ('partial', 'awaiting_signature') THEN 'partial'
   ...
   WHERE id = _invoice_id
     AND (
       ...
       OR (new_state = 'complete' AND delivery_status IS DISTINCT FROM 'delivered')
       OR (delivery_status IS DISTINCT FROM 'delivered' AND new_state IN ('partial', 'awaiting_signature') AND delivery_status IS DISTINCT FROM 'partial')
     );
...
WITH recalculated AS (
  SELECT public.compute_invoice_delivery_state_v2(id) AS state
  FROM public.invoices
  WHERE status NOT IN ('draft', 'voided')
)
SELECT count(*) FROM recalculated;