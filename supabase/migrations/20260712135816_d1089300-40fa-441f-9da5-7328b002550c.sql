...
CREATE OR REPLACE FUNCTION public.get_product_cost_book(
  p_fy_start timestamp with time zone DEFAULT NULL,
  p_fy_end   timestamp with time zone DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  lots AS (
    SELECT
      ...
      (ov.customs_egp + ov.taxes_egp + ov.shipping_egp + ov.other_egp) AS overheads_egp,
      CASE
  ...
  lots_calc AS (
    SELECT
      ...
      (l.qty * l.unit_usd * l.usd_rate) + (l.line_share * l.overheads_egp) AS landed_line_egp
    FROM lots l
  ),
  lots_full AS (
    SELECT
