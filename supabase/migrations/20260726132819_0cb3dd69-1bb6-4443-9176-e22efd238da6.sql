...
BEGIN
  ...
  IF v_status IN ('voided','archived') THEN RETURN; END IF;

  SELECT COUNT(*) INTO v_receipt_count
    FROM public.delivery_receipts WHERE invoice_id = _invoice_id;

  -- Legacy protection: invoice already marked delivered AND has zero receipts → leave alone.
  IF v_delivery = 'delivered' AND v_receipt_count = 0 THEN RETURN; END IF;
...
-- Re-run on every non-voided/archived invoice to fix any that were wrongly flipped to delivered.
DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT id FROM public.invoices WHERE status NOT IN ('voided','archived') LOOP
