...
CREATE POLICY "company members read counters"
ON public.company_counters FOR SELECT
TO authenticated
USING (public.is_company_member());

INSERT INTO public.company_counters (id, receipt_seq)
VALUES ('default', GREATEST(
  19,
  COALESCE((SELECT MAX(receipt_number) FROM public.invoices), 0)
))
ON CONFLICT (id) DO UPDATE
SET receipt_seq = GREATEST(public.company_counters.receipt_seq, EXCLUDED.receipt_seq);
...
BEGIN
  ...
  -- Shared company-wide receipt counter (atomic)
  ...
  -- Keep per-user counter in sync (kept for backward compat, not used in number)
