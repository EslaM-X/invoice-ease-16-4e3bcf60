...
AS $function$
  ...
  path_a AS (
    SELECT l.dri_id, ii.id AS invoice_item_id, ii.product_id, l.qty::bigint AS qty
    FROM dr_lines l
  ...
  resolved AS (
    SELECT dri_id, invoice_item_id, product_id, qty FROM path_a
    UNION ALL
    SELECT dri_id, invoice_item_id, product_id, qty FROM path_b
  ),
  delivered_per_item AS (
    ...
    GROUP BY r.invoice_item_id, r.product_id, ii.quantity
  )
  SELECT product_id, SUM(qty)::bigint AS delivered_qty
  FROM delivered_per_item
...
BEGIN
  ...
  SELECT t.product_id, (t.new_qty - t.old_qty),
    'RECONCILE: rebuilt from PO receipts − capped delivery receipts (active invoices only)', v_actor, now()
...
RETURNS TABLE(product_id uuid, product_name text, serial_number text, received bigint, delivered_counted bigint, delivered_excluded_by_invoice_status bigint, sold bigint, stock_quantity_now integer, expected_stock integer, delta integer)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
  ...
  active_best AS (SELECT dri_id, MAX(score) AS top FROM active_candidates GROUP BY dri_id),
  active_filtered AS (SELECT c.* FROM active_candidates c JOIN active_best b ON b.dri_id = c.dri_id AND b.top = c.score),
  active_path_b AS (
    SELECT
      f.dri_id,
      f.invoice_item_id,
      f.product_id,
      (f.qty::numeric * (f.ii_qty::numeric / NULLIF(SUM(f.ii_qty) OVER (PARTITION BY f.dri_id), 0)))::bigint AS qty
    FROM active_filtered f
  ),
  active_resolved AS (
    SELECT dri_id, invoice_item_id, product_id, qty FROM active_path_a
    UNION ALL
    SELECT dri_id, invoice_item_id, product_id, qty FROM active_path_b
  ),
  counted AS (
    SELECT r.product_id, SUM(LEAST(ii.quantity, r.raw_qty))::bigint AS qty
    FROM (
      SELECT invoice_item_id, product_id, SUM(qty)::bigint AS raw_qty
      FROM active_resolved
      GROUP BY invoice_item_id, product_id
    ) r
    JOIN public.invoice_items ii ON ii.id = r.invoice_item_id
  ...
  excluded AS (
    ...
    WHERE COALESCE(dr.status,'') <> 'cancelled'
      AND COALESCE(i.status,'') IN ('draft','voided','cancelled')
