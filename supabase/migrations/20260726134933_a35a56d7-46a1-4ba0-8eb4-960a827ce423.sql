...
BEGIN
  ...
  -- Hard safety rail: never downgrade invoices already closed/archived by delivery.
  -- This protects legacy closed invoices and prevents stock/report side effects from reopening them.
  IF v_delivery = 'delivered' THEN
    RETURN;
  END IF;

  -- Never calculate cancelled/void/archive-status rows.
