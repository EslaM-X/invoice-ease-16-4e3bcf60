...
BEGIN
  ...
  po_totals AS (
    SELECT po_id, SUM(qty*unit_usd) AS base_usd, SUM(qty) AS total_qty, SUM(qty*unit_weight_g) AS total_weight_g
    FROM po_items GROUP BY po_id
  ),
  po_overheads AS (
    SELECT
      pi.po_id, pi.usd_rate, pt.base_usd, pt.total_qty, pt.total_weight_g,
  ...
  lots AS (
    SELECT
      pi.product_id, pi.qty, pi.unit_usd, pi.unit_weight_g, pi.usd_rate,
  ...
  lots_calc AS (
    SELECT
      ...
      -- Taxes intentionally EXCLUDED from overheads_egp and landed cost.
      -- They are informational only and still returned per-lot for display.
      (l.customs_egp + l.shipping_egp + l.other_egp) AS overheads_egp,
      (l.qty * l.unit_usd * l.usd_rate) AS raw_line_egp,
      (l.qty * l.unit_usd * l.usd_rate)
        + (l.value_share  * (l.customs_egp + l.other_egp))
  ...
  per_product AS (
    SELECT
      ...
      jsonb_agg(
        jsonb_build_object(
          'po_id', po_id, 'po_number', po_number, 'shipment_code', shipment_code,
          'shipment_date', shipment_date, 'status', status,
          'qty', qty, 'unit_usd', unit_usd, 'unit_weight_g', unit_weight_g,
          'usd_rate', usd_rate, 'unit_egp', unit_egp,
          'line_total_egp', qty * unit_egp,
          'customs_egp', customs_egp, 'taxes_egp', taxes_egp,
          'shipping_egp', shipping_egp, 'other_egp', other_egp,
          'overheads_egp', overheads_egp,
          'line_share', line_share, 'value_share', value_share, 'weight_share', weight_share,
          'landed_unit_usd', landed_unit_usd, 'landed_unit_egp', landed_unit_egp,
          'landed_line_egp', landed_line_egp
        ) ORDER BY shipment_date ASC
      ) AS lots
    FROM lots_full GROUP BY product_id
  )
  SELECT jsonb_build_object(
    'default_rate', v_default_rate,
    'products', COALESCE(jsonb_object_agg(product_id::text, jsonb_build_object(
      'total_qty', total_qty, 'wac_usd', wac_usd, 'wac_egp', wac_egp,
      'wac_landed_usd', wac_landed_usd, 'wac_landed_egp', wac_landed_egp,
      'total_landed_egp', total_landed_egp,
      'min_usd', min_usd, 'max_usd', max_usd,
      'latest_usd', latest_usd, 'latest_egp', latest_egp,
      'lots', lots
    )), '{}'::jsonb)
  ) INTO v_result FROM per_product;
