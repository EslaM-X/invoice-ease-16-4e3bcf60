-- Preserve invoice-specific edits for product-backed line items.
-- Stock is still validated against the linked product, but the saved invoice row
-- keeps the user-edited unit price / serial / color snapshot across all accounts.

DROP FUNCTION IF EXISTS public.create_invoice(uuid, numeric, text, text, jsonb);
DROP FUNCTION IF EXISTS public.update_invoice(uuid, uuid, numeric, text, text, jsonb);
...
GRANT EXECUTE ON FUNCTION public.create_invoice(uuid, numeric, text, text, jsonb, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_invoice(uuid, uuid, numeric, text, text, jsonb, numeric) TO authenticated;
